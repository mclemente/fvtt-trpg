...
export default class Item5e extends Item {
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      ...
      props.push(
        game.i18n.localize(data.equipped ? "TRPG.Equipped" : "TRPG.Unequipped"),
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.TRPG.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("TRPG.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.TRPG.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.TRPG.consumableTypes[data.consumableType],
      data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("TRPG.Charges")
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.TRPG.abilities[data.ability] || null,
      CONFIG.TRPG.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("TRPG.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("TRPG.AbbreviationLbs") : null
    );
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("TRPG.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "TRPG.ItemRechargeSuccess" : "TRPG.ItemRechargeFailure")}`,
